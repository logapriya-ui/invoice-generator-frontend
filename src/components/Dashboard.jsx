...
export default function Dashboard() {
  ...
  // --- STATUS TOGGLE (FIXED with optimistic update + revert on failure) ---
  const handleStatusUpdate = async (id, currentStatus, e) => {
    e.stopPropagation();
    const newStatus = currentStatus?.toLowerCase() === "paid" ? "Unpaid" : "Paid";

    // Optimistically update UI immediately so user sees change at once
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setHistory(prev => prev.map(inv => inv._id === id ? { ...inv, status: currentStatus } : inv));
  ...
  // --- STAT CALCULATIONS ---
  const paidCount = history.filter(d => d.status?.toLowerCase() === 'paid').length;
  const unpaidCount = history.length - paidCount;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      <aside className="relative w-full md:w-80 p-8 border-r bg-white shadow-sm h-screen sticky top-0">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-4 bg-slate-100 hover:bg-blue-50 hover:text-blue-700 rounded-xl text-xs font-black text-slate-600 transition-all"
          >
          ...
          <div className="relative">
            <button
              ...
              className="w-full bg-blue-900 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg"
            >
              ...
              <div className="absolute top-full left-0 w-full mt-2 bg-white shadow-2xl rounded-2xl border p-2 z-50">
                {['Invoice', 'Quote', 'Credit Note'].map(type => (
                  <button
                    ...
                    className="w-full text-left p-3 hover:bg-blue-50 rounded-lg text-sm font-bold text-slate-700 flex items-center gap-3"
                  >
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          ...
          {/* STATS CARDS — now 4 cards including Paid/Unpaid counts */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
            <div className="bg-blue-600 p-6 md:p-8 rounded-3xl text-white relative overflow-hidden">
              <p className="text-xs font-bold opacity-80 uppercase">Total Docs</p>
              <h3 className="text-4xl font-black mt-1">{history.length}</h3>
              <FileText className="absolute -right-4 -bottom-4 w-24 h-24 opacity-10" />
            </div>
            <div className="bg-emerald-500 p-6 md:p-8 rounded-3xl text-white relative overflow-hidden">
              <p className="text-xs font-bold opacity-80 uppercase">Revenue</p>
              <h3 className="text-2xl md:text-3xl font-black mt-1">
                ₹{history.reduce((acc, curr) => acc + (curr.total || 0), 0).toLocaleString()}
              </h3>
              <IndianRupee className="absolute -right-4 -bottom-4 w-24 h-24 opacity-10" />
            </div>
            <div className="bg-white border-2 border-emerald-200 p-6 rounded-3xl">
              <p className="text-xs font-bold text-slate-400 uppercase">Paid</p>
              <h3 className="text-4xl font-black mt-1 text-emerald-600">{paidCount}</h3>
            </div>
            <div className="bg-white border-2 border-red-200 p-6 rounded-3xl">
              <p className="text-xs font-bold text-slate-400 uppercase">Unpaid</p>
              <h3 className="text-4xl font-black mt-1 text-red-500">{unpaidCount}</h3>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border-2 border-slate-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b bg-slate-50/50 flex justify-between items-center">
              ...
              <span className="text-[10px] text-slate-400 font-bold hidden md:block">Click status badge to toggle Paid / Unpaid</span>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-20 text-center">
                ...
                <p className="font-black text-slate-400">No documents yet.</p>
                <p className="text-xs text-slate-300 mt-1">Click "New Document" to create your first invoice.</p>
              </div>
            ) : (
                      ...
                      return (
                        <tr key={doc._id} className="hover:bg-blue-50/30 transition-colors group cursor-default">
                          <td className="px-6 py-4">
                            {/* FIXED: Colored status badge, click to toggle */}
                            <button
                              type="button"
